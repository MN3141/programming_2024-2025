...
FROM facultate
JOIN specializare
ON facultate.id_facultate =
specializare.id_facultate;

-- Listează ID-ul și numele specializărilor de la facultatea X (alege tu stringul X).
...
WHERE 
    facultate.denumire = 'Facultatea de Inginerie';
    
    
-- Afișează numele specializării care are cele mai multe cursuri. (opțional,
-- rezolvati prin doua metode)
select * from specializare;
select * from materie;
SELECT 
    specializare.denumire AS nomen, 
    COUNT(materie.id_materie) AS nr
FROM 
    specializare
JOIN 
    materie ON specializare.id_specializare = materie.id_specializare
GROUP BY 
    specializare.denumire
ORDER BY 
    COUNT(materie.id_materie) DESC
LIMIT 1;