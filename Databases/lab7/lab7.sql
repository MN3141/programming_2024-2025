...
-- Listează ID-ul și denumirea specializărilor care aparțin facultăților fondate
-- înainte de anul 1920.

-- A:Join method
SELECT 
    specializare.id_specializare as id,
    specializare.denumire as nomen
FROM 
    specializare
JOIN 
    facultate 
...
FROM 
    specializare
WHERE id_facultate in (
	SELECT id_facultate FROM
    facultate
    WHERE facultate.an_fondare <1920)
    
-- Setează data examenului anul viitor, de ziua Europei, pentru toate notele
-- corespunzătoare materiilor predate în anul 2 de studiu.

-- A:Join method
UPDATE nota n
JOIN materie m ON n.id_materie = m.id_materie
SET n.data_examen = DATE(CONCAT(YEAR(CURDATE()) + 1, '-05-09'))
WHERE m.an_de_studiu = 2;

-- Șterge toate persoanele care nu au adresa completată corect (NULL sau mai
-- mică de 10 caractere) și nu sunt studenți înmatriculați
